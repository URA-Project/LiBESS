...
class Population:
    def __init__(self, supply_orders=None, session_capacity_threshold=None, population_size=0):
        ...
        # Nếu supply_orders là list (danh sách individuals)
        if isinstance(supply_orders, list):
            self.individuals = supply_orders
            return
            
        # Nếu supply_orders là DataFrame
        if (supply_orders is not None 
            and isinstance(supply_orders, pd.DataFrame)  # Thêm kiểm tra kiểu DataFrame
            and not supply_orders.empty 
            and population_size > 0):
            
            self.initialize_population(supply_orders, session_capacity_threshold, population_size)

    def initialize_population(self, supply_orders, session_capacity_threshold, population_size):
