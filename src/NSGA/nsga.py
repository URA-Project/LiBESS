...
def selection(population_size, front, chroms_obj_record, total_chromosome):
    ...
    population_list = []
    for n in new_pop:
        temp_chromosome = total_chromosome[n]
        chromosome = total_chromosome[n].chromosome
        temp_chromosome.chromosome = np.asarray(chromosome)
        population_list.append(temp_chromosome)

    return np.array(population_list), new_pop
...
def save_output(chromosome, index):
    ...
    if ((index + 1) % 10 == 0) or (index == 0):
        output_file = "/Users/mac/Library/CloudStorage/OneDrive-Personal/Study/URA/GA_Emerging_Papers/LiBESS-MAIN/output/output_nsga.txt"
